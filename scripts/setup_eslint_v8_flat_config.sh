...
pnpm pkg set 'scripts.eslint'="ESLINT_USE_FLAT_CONFIG=true npx eslint --config eslint.config.mjs ."

settings_file=".vscode/settings.json"

## Need to setup visual studio code to use eslint flat config
# Check if the settings file already exists
if [ ! -f "$settings_file" ]; then
    echo "settings.json does not exist. Creating..."
    ...
    # Create the settings.json file with an empty JSON object
    echo "{}" > "$settings_file"
fi

# Add the eslint.experimental.useFlatConfig setting to the settings file
jq '.["eslint.experimental.useFlatConfig"] = true' "$settings_file" > temp.json && mv temp.json "$settings_file"

# Add the override for the config file.
jq 'if has("eslint.options") then 
